      ...
      query: async (sql: string, params: any[] = []) => {
        ...
        if (sql.includes('INSERT INTO tasks')) {
          const taskData = {
            ...
            type: params[5] || 'immediate',
            scheduledTime: params[6] || null,
            estimatedDuration: params[7] || null,
            createdAt: params[8] || new Date().toISOString(),
            updatedAt: params[9] || new Date().toISOString(),
            recurringPattern: params[10] || null,
            dueDate: params[11] || null,
            imageUrl: null
          };
...
if (typeof window !== 'undefined' && !window.electronAPI) {
  ...
  mockAPI.database.query(
    'INSERT INTO tasks (title, description, category, priority, status, type, scheduledTime, estimatedDuration, createdAt, updatedAt, recurringPattern, dueDate)',
    ['サンプルタスク1', '最初のサンプルタスクです', 'work', 'high', 'pending', 'immediate', null, 30, new Date().toISOString(), new Date().toISOString(), null, null]
  );
  
  mockAPI.database.query(
    'INSERT INTO tasks (title, description, category, priority, status, type, scheduledTime, estimatedDuration, createdAt, updatedAt, recurringPattern, dueDate)',
    ['サンプルタスク2', '完了済みのタスクです', 'health', 'medium', 'completed', 'immediate', null, 15, new Date().toISOString(), new Date().toISOString(), null, null]
  );
}
